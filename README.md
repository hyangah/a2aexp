# A2a Exp Go API Library

<!-- x-release-please-start-version -->

<a href="https://pkg.go.dev/github.com/stainless-sdks/a2a-exp-go"><img src="https://pkg.go.dev/badge/github.com/stainless-sdks/a2a-exp-go.svg" alt="Go Reference"></a>

<!-- x-release-please-end -->

The A2a Exp Go library provides convenient access to the [A2a Exp REST API](https://github.com/google/a2a-spec)
...
```go
import (
	"github.com/stainless-sdks/a2a-exp-go" // imported as a2aexp
)
```
...
```sh
go get -u 'github.com/stainless-sdks/a2a-exp-go@v0.0.1'
```
...
func main() {
	client := a2aexp.NewClient(
		option.WithAPIKey("My API Key"), // defaults to os.LookupEnv("A2A_EXP_API_KEY")
	)
...
The a2aexp library uses the [`omitzero`](https://tip.golang.org/doc/go1.24#encodingjsonpkgencodingjson)
semantics from the Go 1.24+ `encoding/json` release for request fields.

Required primitive fields (`int64`, `string`, etc.) feature the tag <code>\`json:"...,required"\`</code>. These
fields are always serialized, even their zero values.

Optional primitive types are wrapped in a `param.Opt[T]`. These fields can be set with the provided constructors, `a2aexp.String(string)`, `a2aexp.Int(int64)`, etc.
...
```go
p := a2aexp.ExampleParams{
	ID:   "id_xxx",             // required property
	Name: a2aexp.String("..."), // optional property

	Point: a2aexp.Point{
		X: 0,             // required field will serialize as 0
		Y: a2aexp.Int(1), // optional field will serialize as 1
		// ... omitted non-required fields will not be serialized
	},

	Origin: a2aexp.Origin{}, // the zero value of [Origin] is considered omitted
}
```
...
custom := param.Override[a2aexp.FooParams](12)
```
...
See the [full list of request options](https://pkg.go.dev/github.com/stainless-sdks/a2a-exp-go/option).
...
`*a2aexp.Error`. This contains the `StatusCode`, `*http.Request`, and
...
We also provide a helper `a2aexp.File(reader io.Reader, filename string, contentType string)`
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/a2a-exp-go/issues) with questions, bugs, or suggestions.
